...
cursor = conn.cursor()
destroyTable = "DROP TABLE IF EXISTS train_set"
cursor.execute(destroyTable)
command = '''CREATE TABLE train_set(id integer PRIMARY KEY, question VARCHAR(255), question_id integer, train_id integer, statement text, name_entity text[], name_entity_claim text[],
...
def createDataSet(question_id, count, id_list, question, conn, cursor, classified_as):
    for stat_id in id_list:
        ...
        cursor.execute('''INSERT INTO train_set Values (%s,%s,%s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''', (count, question, question_id, stat_id, claim, name_entity, name_entity_claim, name_entity_sum,
...
for index in range(df.shape[0]):
    ...
    # false_id_list = df.false_id[index]
    ...
    #
    #
    #     cursor.execute('''INSERT INTO train_set Values (%s,%s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''', (count, question, stat_id, claim, name_entity, name_entity_claim, name_entity_sum,
    #     ques_length, ques_score, stat_length, float(ques_noun)/ques_length, float(ques_verb)/ques_length, float(ques_adj)/ques_length, speaker_half_match, speaker_full_match, score/ques_score, highest_noun, second_noun, highest_verb, second_verb, highest_adj, second_adj, float(sum_others)/ques_score, float(person_entity)/ques_length, float(gpe_entity)/ques_length, float(person_entity_match)/ques_length,
    #     float(gpe_entity_match)/ques_length, float(org_entity)/ques_length, float(org_entity_match)/ques_length, float(full_other_match)/ques_length, float(sum_score)/ques_score, 1))
    #     conn.commit()
    #     count += 1


    # random_ids = []
    # if len(false_id_list) >=5:
    #     for i in range(5):
    #         random_ids.append(false_id_list[i])
    #     for i in range(5):
    #         rand_id = random.randint(-1, len(potential_id) - 1)
    #         while potential_id[rand_id] in stat_id_list:
    #             rand_id = random.randint(-1, len(potential_id) - 1)
    #         random_ids.append(potential_id[rand_id])
    # else:
    #     for i in range(len(false_id_list)):
    #         random_ids.append(false_id_list[i])
    #     for i in range(10-len(false_id_list)):
    #         rand_id = random.randint(-1, len(potential_id) - 1)
    #         while potential_id[rand_id] in stat_id_list:
    #             rand_id = random.randint(-1, len(potential_id) - 1)
    #         random_ids.append(potential_id[rand_id])
    #
    # for false_id in random_ids:
    #     cursor.execute("select statement, speaker, summary from facts where id = %s", (false_id,))
    #     result_row = cursor.fetchone()
    ...
    #
    #
    #     cursor.execute('''INSERT INTO train_set Values (%s,%s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''', (count, question, false_id, claim, name_entity, name_entity_claim, name_entity_sum,
    #     ques_length, ques_score, stat_length, float(ques_noun)/ques_length, float(ques_verb)/ques_length, float(ques_adj)/ques_length, speaker_half_match, speaker_full_match, score/ques_score, highest_noun, second_noun, highest_verb, second_verb, highest_adj, second_adj, float(sum_others)/ques_score, float(person_entity)/ques_length, float(gpe_entity)/ques_length, float(person_entity_match)/ques_length,
    #     float(gpe_entity_match)/ques_length, float(org_entity)/ques_length, float(org_entity_match)/ques_length, float(full_other_match)/ques_length, float(sum_score)/ques_score, 0))
    #     count += 1
